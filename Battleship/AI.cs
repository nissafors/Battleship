...
namespace Battleship
{
    class AI
    {
        ...
        private void AIChooseShot(Battleship.Square[,] playFieldArray, out int xCoord, out int yCoord)
        {
            ...
            for (int y = 0; y < maxY && !foundHit; y++)
            {
                for (int x = 0; x < maxX && !foundHit; x++)
                {
                    ...
                    if (playFieldArray[x, y] == Battleship.Square.Hit)
                    {
                        ...
                        if (x < (maxX - 1))
                        {
                            if (playFieldArray[x + 1, y] == Battleship.Square.Hit)
                            {
                                ...
                                while (x < (maxX - 1) && playFieldArray[x, y] == Battleship.Square.Hit)
                                {
                                    x++;
                                }

                                // Add the square right of the last hit unless we are at the edge or there is a miss
                                if (playFieldArray[x, y] == Battleship.Square.Water || playFieldArray[x, y] == Battleship.Square.Ship)
                                {
                                    allowedCoordinates.Add(Tuple.Create(x - 1, y));
                                }
                                foundHit = true;
                        ...
                        if (y < (maxY - 1))
                        {
                            if (playFieldArray[x, y + 1] == Battleship.Square.Hit)
                            {
                        ...
                        if (!foundHit)
                        {
                            ...
                            if (x != minX && playFieldArray[x - 1, y] != Battleship.Square.Miss)
                            {
                                allowedCoordinates.Add(Tuple.Create(x - 1, y));
                            }
                            if (y != minY && playFieldArray[x, y - 1] != Battleship.Square.Miss)
                            {
                                allowedCoordinates.Add(Tuple.Create(x, y - 1));
                            }
                            if (x < (maxX - 1) && playFieldArray[x + 1, y] != Battleship.Square.Miss)
                            {
                                allowedCoordinates.Add(Tuple.Create(x + 1, y));
                            }
                            if (y < (maxY - 1) && playFieldArray[x, y + 1] != Battleship.Square.Miss)
                            {
                                allowedCoordinates.Add(Tuple.Create(x, y + 1));
                            }
                            foundHit = true;
                    ...
                    else if ((playFieldArray[x, y] == Battleship.Square.Water || playFieldArray[x, y] == Battleship.Square.Ship) && !foundHit)
                    {
                        // Check above and left of the current element for a sunk ship
                        if (x != minX && y != minY && playFieldArray[x - 1, y - 1] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check above the current element for a sunk ship
                        else if (y != minY && playFieldArray[x, y - 1] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check above and right of the current element for a sunk ship
                        else if (x < (maxX - 1) && y != minY && playFieldArray[x + 1, y - 1] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check to the left of the current element for a sunk ship
                        else if (x != minX && playFieldArray[x - 1, y] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check to the right of the current element for a sunk ship
                        else if (x < (maxX - 1) && playFieldArray[x + 1, y] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check below and left of the current element for a sunk ship
                        else if (x != minX && y < (maxY - 1) && playFieldArray[x - 1, y + 1] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check below the current element for a sunk ship
                        else if (y < (maxY - 1) && playFieldArray[x, y + 1] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }

                        // Check below and left of the current element for a sunk ship
                        else if (x < (maxX - 1) && y < (maxY - 1) && playFieldArray[x + 1, y + 1] == Battleship.Square.Sunk)
                        {
                            addCoord = false; // Ships can't be next to each other
                        }
